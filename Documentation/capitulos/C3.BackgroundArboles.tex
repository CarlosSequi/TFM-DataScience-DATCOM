\chapter[Capítulo 3. Contexto en algoritmos de árboles de decisión]{Contexto en algoritmos de árboles de decisión}
...
Una aproximación natural a RSS es el '\textbf{ratio de error en la clasificación}', basado simplemente en la fracción de las observaciones de training en dicha región que no pertenecen a la clase más común de esta \cite{ref7}. Se calcula de la siguiente forma:
...
Sabiendo que $\delta$ es la tolerancia al fallo a la hora de escoger un atributo en un nodo dado\cite{ref10}.\\
...
Un algoritmo que utiliza este concepto de ventana deslizante es el llamado\textbf{ Concept-adapting Very Fast Decision Tree (CVFDT)} que, evidentemente, contiene además las características del VFDT. A continuación la explicación de su funcionamiento\cite{refff12}:
